...
class LRAM(nn.Module):
    def __init__(self, in_features, phi, kernel_size=None):
...
def train_baseline_model(model_type, subID, data_type='obj', max_epochs=1000, patience=100):
    ...
    if model_type in ['RandomForest', 'SVM']:
        ...
        elif model_type == 'PODAR':
            model = PODAR(horizon=hor).to(device)
        else:
        ...
        lr = 0.01 if model_type == 'PODAR' else 0.001
...
def comprehensive_baseline_comparison(num_runs=3):
    baseline_models = ['CNN_LSTM', 'MultiHeadAttention', 'VanillaRNN', 'VanillaLSTM',
                       'BiLSTM', 'GRU', 'Transformer', 'RandomForest', 'SVM', 'PODAR']
    data_types = ['obj', 'sub']

    all_results = {}

    print("开始全面基线模型比较（已包含PODAR作为原始SOTA基线）...")
    print("=" * 100)
    print(
...
def convert_to_serializable(obj):
    """递归地将 NumPy 类型转换为 Python 原生类型，便于 JSON 序列化"""
...
class AblationPLMA(nn.Module):
    def __init__(self, horizon, config):
        super().__init__()
        self.config = config
        self.horizon_val = int(horizon * 10) if horizon is not None else None
        self.use_horizon = config.get('use_horizon', True)
        self.horizon = self.horizon_val if self.use_horizon else None

        # 共享物理参数
        self.m_ego = torch.tensor(1.8, device=device)
        self.m_obj = torch.tensor(1.8, device=device)
        self.Alpha_ = nn.Parameter(torch.tensor([0.7]))
        self.A = nn.Parameter(torch.tensor([0.3]))
        self.B = nn.Parameter(torch.tensor([0.5]))
        self.scale = nn.Parameter(torch.tensor([0.5 / 11.25]))

        # 条件模块
        if config['use_lram']:
            kernel_size = config.get('lram_kernel', 5)
            self.lram = LRAM(in_features=1, phi='T', kernel_size=kernel_size)
        else:
            self.lram = None
        ...
        self.mean_pool = config.get('mean_pool', False)
        ...
        # Alpha 可训练性
        if not config.get('alpha_trainable', True):
            self.Alpha_.requires_grad = False
            if 'fixed_alpha' in config:
                self.Alpha_.data = torch.tensor([config['fixed_alpha']])

        # 固定 A/B（无衰减）
        if config.get('fixed_ab', False):
            self.A.requires_grad = False
            self.B.requires_grad = False
            self.A.data = torch.tensor([0.0])
    ...
    def forward(self, delta_v, abs_v, i, d):
        ...
        # 输入特征消融
        if config['features'] == 'wo_delta_v':
            delta_v = torch.zeros_like(delta_v)
        elif config['features'] == 'wo_abs_v':
            abs_v = torch.zeros_like(abs_v)
        elif config['features'] == 'no_time':
        ...
        # 速度融合
        v = self.Alpha_ * delta_v + (1 - self.Alpha_) * abs_v
        damage = v * v.abs() * m * 0.001 * self.scale

        # LRAM
        if self.lram is not None:
            damage = self.lram(damage)
        ...
        # MFAFM 或 Mean Pooling
        if self.mfafm is not None:
            damage, attention_weights = self.mfafm(damage)
            self.attention_weights = attention_weights
        elif self.mean_pool:
            damage = damage.mean(dim=1, keepdim=True)  # (b, 1)

        # 衰减权重
        A_clamped = torch.clamp(self.A, 0.17, 50)
        B_clamped = torch.clamp(self.B, 0.0, 50)
        w_i = torch.exp(-A_clamped * i)
        w_d = torch.exp(-B_clamped * d)
        attenu = w_i * w_d

        podar_t = damage * attenu

        # Horizon 裁剪
        if self.horizon is not None:
            podar = torch.max(podar_t[:, :self.horizon], dim=1)[0]
        else:
            podar = torch.max(podar_t, dim=1)[0]

        self.p_max = podar.max()
        return podar


def run_ablation_study(subID=0, data_type='obj', max_epochs=20000, patience=500, num_runs=3):
    """
    完整扩展消融实验 - 包含正则化、输入特征、参数敏感性等多种变体
    运行所有8个subject并取平均结果（推荐）
    """
    print(f"🔬 开始完整扩展消融实验 - Subject {subID} (若需全subject可循环调用), Type: {data_type}")
    ...
    experiments = {
        'Full PLMA': {
            'use_lram': True, 'use_mfafm': True, 'dropout_rate': None, 'weight_decay': 0.0,
            'features': 'full', 'alpha_trainable': True, 'lram_kernel': 5, 'use_horizon': True
        },
        'w/o LRAM': {
            'use_lram': False, 'use_mfafm': True, 'dropout_rate': None, 'weight_decay': 0.0,
            'features': 'full', 'alpha_trainable': True, 'lram_kernel': 5, 'use_horizon': True
        },
        'w/o MFAFM': {
            'use_lram': True, 'use_mfafm': False, 'dropout_rate': None, 'weight_decay': 0.0,
            'features': 'full', 'alpha_trainable': True, 'lram_kernel': 5, 'use_horizon': True
        },
        'MFAFM → Mean Pooling': {
            'use_lram': True, 'use_mfafm': False, 'mean_pool': True, 'dropout_rate': None, 'weight_decay': 0.0,
            'features': 'full', 'alpha_trainable': True, 'lram_kernel': 5, 'use_horizon': True
        },
        'Dropout 0.1': {
            'use_lram': True, 'use_mfafm': True, 'dropout_rate': 0.1, 'weight_decay': 0.0,
            'features': 'full', 'alpha_trainable': True, 'lram_kernel': 5, 'use_horizon': True
        },
        'Dropout 0.3': {
            'use_lram': True, 'use_mfafm': True, 'dropout_rate': 0.3, 'weight_decay': 0.0,
            'features': 'full', 'alpha_trainable': True, 'lram_kernel': 5, 'use_horizon': True
        },
        'Dropout 0.5': {
            'use_lram': True, 'use_mfafm': True, 'dropout_rate': 0.5, 'weight_decay': 0.0,
            'features': 'full', 'alpha_trainable': True, 'lram_kernel': 5, 'use_horizon': True
        },
        'Weight Decay 1e-5': {
            'use_lram': True, 'use_mfafm': True, 'dropout_rate': None, 'weight_decay': 1e-5,
            'features': 'full', 'alpha_trainable': True, 'lram_kernel': 5, 'use_horizon': True
        },
        'Weight Decay 1e-3': {
            'use_lram': True, 'use_mfafm': True, 'dropout_rate': None, 'weight_decay': 1e-3,
            'features': 'full', 'alpha_trainable': True, 'lram_kernel': 5, 'use_horizon': True
        },
        'Dropout 0.3 + WD 1e-4': {
            'use_lram': True, 'use_mfafm': True, 'dropout_rate': 0.3, 'weight_decay': 1e-4,
            'features': 'full', 'alpha_trainable': True, 'lram_kernel': 5, 'use_horizon': True
        },
        'w/o delta_v': {
            'use_lram': True, 'use_mfafm': True, 'dropout_rate': None, 'weight_decay': 0.0,
            'features': 'wo_delta_v', 'alpha_trainable': True, 'lram_kernel': 5, 'use_horizon': True
        },
        'w/o abs_v': {
            'use_lram': True, 'use_mfafm': True, 'dropout_rate': None, 'weight_decay': 0.0,
            'features': 'wo_abs_v', 'alpha_trainable': True, 'lram_kernel': 5, 'use_horizon': True
        },
        'w/o time decay (i)': {
            'use_lram': True, 'use_mfafm': True, 'dropout_rate': None, 'weight_decay': 0.0,
            'features': 'no_time', 'alpha_trainable': True, 'lram_kernel': 5, 'use_horizon': True
        },
        'w/o distance decay (d)': {
            'use_lram': True, 'use_mfafm': True, 'dropout_rate': None, 'weight_decay': 0.0,
            'features': 'no_dist', 'alpha_trainable': True, 'lram_kernel': 5, 'use_horizon': True
        },
        'Only velocity': {
            'use_lram': True, 'use_mfafm': True, 'dropout_rate': None, 'weight_decay': 0.0,
            'features': 'velocity_only', 'alpha_trainable': True, 'lram_kernel': 5, 'use_horizon': True
        },
        'Fixed Alpha=0.5': {
            'use_lram': True, 'use_mfafm': True, 'dropout_rate': None, 'weight_decay': 0.0,
            'features': 'full', 'alpha_trainable': False, 'fixed_alpha': 0.5, 'lram_kernel': 5, 'use_horizon': True
        },
        'Fixed Alpha=1.0': {
            'use_lram': True, 'use_mfafm': True, 'dropout_rate': None, 'weight_decay': 0.0,
            'features': 'full', 'alpha_trainable': False, 'fixed_alpha': 1.0, 'lram_kernel': 5, 'use_horizon': True
        },
        'Fixed A/B=0 (no decay)': {
            'use_lram': True, 'use_mfafm': True, 'dropout_rate': None, 'weight_decay': 0.0,
            'features': 'full', 'alpha_trainable': True, 'lram_kernel': 5, 'use_horizon': True, 'fixed_ab': True
        },
        'LRAM kernel=3': {
            'use_lram': True, 'use_mfafm': True, 'dropout_rate': None, 'weight_decay': 0.0,
            'features': 'full', 'alpha_trainable': True, 'lram_kernel': 3, 'use_horizon': True
        },
        'LRAM kernel=9': {
            'use_lram': True, 'use_mfafm': True, 'dropout_rate': None, 'weight_decay': 0.0,
            'features': 'full', 'alpha_trainable': True, 'lram_kernel': 9, 'use_horizon': True
        },
        'No Horizon Clipping': {
            'use_lram': True, 'use_mfafm': True, 'dropout_rate': None, 'weight_decay': 0.0,
            'features': 'full', 'alpha_trainable': True, 'lram_kernel': 5, 'use_horizon': False
        },
    }
    ...
    for exp_name, config in experiments.items():
        print(f"  ⚡ 训练 {exp_name} ({num_runs} 次重复取平均)...")
        run_metrics = []
        ...
        for run in range(num_runs):
            ...
            # 使用新的消融类
            net = AblationPLMA(horizon=hor, config=config).to(device)
            net.apply(init_weights)

            optimizer = torch.optim.Adam(net.parameters(), lr=0.01, weight_decay=config['weight_decay'])
            criteria = nn.MSELoss()
            early_stopping = EarlyStopping(patience=patience, min_delta=1e-6)
        ...
        # 取平均
        if run_metrics:
            avg_metrics = {k: np.mean([m[k] for m in run_metrics]) for k in run_metrics[0]}
            std_metrics = {k: np.std([m[k] for m in run_metrics]) for k in run_metrics[0]}
            ablation_results[exp_name] = {'avg': avg_metrics, 'std': std_metrics}
            print(f"    → 平均 R² = {avg_metrics['r2']:.4f} ± {std_metrics['r2']:.4f}")

    # 修复：先转换为可序列化的格式
    ablation_results = convert_to_serializable(ablation_results)

    # 保存结果
    os.makedirs('scientific_data', exist_ok=True)
    save_path = f'scientific_data/extended_ablation_sub{subID}_{data_type}.json'
    ...
    print(f"✅ 扩展消融实验完成，结果保存至 {save_path}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--baseline_comparison':
        print("开始基线模型比较实验（已包含PODAR作为SOTA基线）...")
        ...
        all_ablation = {}
        for subID in range(8):
            for dtype in ['obj', 'sub']:
                key = f"sub{subID}_{dtype}"
                all_ablation[key] = run_ablation_study(subID=subID, data_type=dtype)
        print("所有subject消融实验完成！")
        ...
        print("\n要运行扩展消融实验：python plam.py --ablation")
        print("要运行基线比较：python plam.py --baseline_comparison")